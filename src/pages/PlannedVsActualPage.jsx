...
// ─── S-Curve tooltip ──────────────────────────────────────────────────────────
const TAG_DOT_COLORS = { Red: '#ef4444', Yellow: '#eab308', Green: '#22c55e', Blue: '#3b82f6', White: '#94a3b8' }

function SCurveTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null

  // Separate bar entries (tag counts) from line entries (CUM values)
  const tagEntries = payload.filter(p => ['Red','Yellow','Green','Blue','White'].includes(p.name))
  const cumEntries = payload.filter(p => p.name === 'Actual CUM' || p.name === 'Target CUM')
  const totalBar   = tagEntries.reduce((s, p) => s + (p.value || 0), 0)
  ...
  return (
    <div style={{
      ...
      {/* Period label */}
      <div style={{ color: '#64748b', fontWeight: 700, marginBottom: 10, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</div>

      {/* Tag counts — only show tags with value > 0 */}
      {tagEntries.filter(p => p.value > 0).map(p => (
        <div key={p.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 9, height: 9, borderRadius: '50%', background: TAG_DOT_COLORS[p.name] || p.fill, flexShrink: 0 }} />
      ...
      {totalBar > 0 && (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 6 }}>
            ...
            <span style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{totalBar}</span>
          </div>
        </>
      )}

      {/* Cumulative lines */}
      {cumEntries.filter(p => p.value != null).map(p => (
        <div key={p.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 3 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 14, height: 2, background: p.color || p.stroke, borderRadius: 1 }} />
            <span style={{ color: '#94a3b8' }}>{p.name}</span>
          </div>
          <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{p.value}</span>
        </div>
...
export default function PlannedVsActualPage() {
  ...
  // chartView mirrors the global period toggle; user can still override it via the chart buttons
  const [chartView, setChartView] = useState(period || 'Overall')
  const fileInputRef = React.useRef(null)

  // Keep chartView in sync when the global period toggle changes
  React.useEffect(() => { setChartView(period || 'Overall') }, [period])
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 24px' }}>
            ...
            {sCurveData.length > 0 ? (
              <ResponsiveContainer width="100%" height={320}>
                <ComposedChart data={sCurveData} margin={{ top: 8, right: 36, bottom: 8, left: 0 }} barCategoryGap="18%">
                  ...
                  {/* Stacked bars — fixed order: Red → Yellow → Green → Blue → White */}
                  <Bar yAxisId="left" dataKey="red"    name="Red"    stackId="tags" fill="#ef4444" fillOpacity={0.85} radius={0} />
                  <Bar yAxisId="left" dataKey="yellow" name="Yellow" stackId="tags" fill="#eab308" fillOpacity={0.85} radius={0} />
                  <Bar yAxisId="left" dataKey="green"  name="Green"  stackId="tags" fill="#22c55e" fillOpacity={0.85} radius={0} />
                  <Bar yAxisId="left" dataKey="blue"   name="Blue"   stackId="tags" fill="#3b82f6" fillOpacity={0.80} radius={0} />
                  <Bar yAxisId="left" dataKey="white"  name="White"  stackId="tags" fill="#94a3b8" fillOpacity={0.55} radius={[2,2,0,0]} />
                  ...
                  <Line yAxisId="right" type="monotone" dataKey="cumTarget" name="Target CUM"  stroke="rgba(100,116,139,0.6)" strokeWidth={2} strokeDasharray="6 4" dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            ) : (
